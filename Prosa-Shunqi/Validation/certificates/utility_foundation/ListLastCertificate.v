...
Lemma ll_filter_canonical_coq (P : nat -> bool) (xs : seq nat) :
  Logic.eq (ll_target_filter (ll_target_pred P) (ll_to_imported xs))
    (ll_to_imported [seq x <- xs | P x]).
Proof.
  induction xs as [|x xs IH].
  - exact (imported_eq_to_coq_eq _ _
      (ImportedListLast.Prosa_Validation_ListLastInterface_filter_nil
        (ll_target_pred P))).
  - refine (Logic.eq_trans
      (imported_eq_to_coq_eq _ _
    ...
    unfold ll_target_pred. rewrite (sub_nat_rocq_roundtrip x).
      ...
      (imported_eq_to_coq_eq _ _
        (ll_filter_branch_canonical (P x) x
          ...
          (ImportedListLast.List_filter_inst1 Lean.Nat
            (fun n => ll_bool_to_imported (P (sub_nat_to_rocq n)))
            (ll_to_imported xs))
          (coq_eq_to_imported_eq _ _ IH))) _).
    exact (f_equal ll_to_imported (ll_source_filter_cons P x xs)).
Qed.
...
Lemma ll_filter_canonical (P : nat -> bool) (xs : seq nat) :
  Lean.eq (ll_target_filter (ll_target_pred P) (ll_to_imported xs))
    (ll_to_imported [seq x <- xs | P x]).
Proof.
  exact (coq_eq_to_imported_eq _ _ (ll_filter_canonical_coq P xs)).
Qed.
...
Proof.
  ...
  apply coq_eq_to_imported_eq. rewrite Hback. reflexivity.
Qed.
...
Definition ll_target_getD
  ...
  ImportedListLast.Prosa_Validation_Rocq90Batch2ListInterface_getD_inst1
    Lean.Nat xs n ll_target_zero.
...
Lemma ll_getD_canonical_coq (xs : seq nat) (n : nat) :
  Logic.eq (ll_target_getD (ll_to_imported xs) (sub_nat_to_imported n))
...
Proof.
  ...
  - rewrite nth_nil. cbn [sub_nat_to_imported ll_to_imported].
    exact (Logic.eq_trans
      (imported_eq_to_coq_eq _ _
        (ImportedListLast.Prosa_Validation_Rocq90Batch2ListInterface_getD_nat_nil
        ...
        (imported_eq_to_coq_eq _ _
          (sub_imported_eq_sym _ _
            (sub_imported_eq_congr
              ...
              ImportedListLast.Prosa_Validation_ListLastInterface_nat_zero)))
        (imported_eq_to_coq_eq _ _
          (ImportedListLast.Prosa_Validation_Rocq90Batch2ListInterface_getD_nat_zero
            (sub_nat_to_imported x) (ll_to_imported xs)))).
    + exact (Logic.eq_trans
        (imported_eq_to_coq_eq _ _
          (ImportedListLast.Prosa_Validation_Rocq90Batch2ListInterface_getD_nat_succ
            (sub_nat_to_imported x) (ll_to_imported xs)
            (sub_nat_to_imported n)))
        (IH n)).
Qed.
...
Lemma ll_getD_canonical (xs : seq nat) (n : nat) :
  Lean.eq (ll_target_getD (ll_to_imported xs) (sub_nat_to_imported n))
    (sub_nat_to_imported (nth O xs n)).
Proof.
  exact (coq_eq_to_imported_eq _ _ (ll_getD_canonical_coq xs n)).
Qed.
...
Proof.
  ...
  - have Hrel := sub_nat_le_correspondence a (sub_nat_to_imported a)
    ...
    apply coq_eq_to_imported_eq. rewrite Hmax. reflexivity.
...
Definition ll_target_lt (a b : Lean.Nat) : SProp :=
  ImportedListLast.Prosa_Validation_Rocq90Batch2ListInterface_natLt a b.
...
Definition ll_target_mem (x : Lean.Nat)
    (xs : ImportedListLast.List_inst1 Lean.Nat) : SProp :=
  ImportedListLast.Prosa_Validation_Rocq90Batch2ListInterface_natMem x xs.
